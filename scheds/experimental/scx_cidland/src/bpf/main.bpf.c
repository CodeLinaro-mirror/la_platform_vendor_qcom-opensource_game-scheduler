		   ...
		   u64 wake_flags)
{
	...
	cid = pick_idle_cid(p, prev_cid, false);
	if (cid < 0)
		return prev_cid;
...
/*
 * Return true if an idle cid should be kicked to pick @p up, false otherwise.
 */
static bool needs_idle_kick(struct task_struct *p, u64 enq_flags)
{
...
void BPF_STRUCT_OPS(cidland_enqueue, struct task_struct *p, u64 enq_flags)
{
	...
	if (!needs_idle_kick(p, enq_flags))
		return;

	scx_bpf_kick_cid(prev_cid, SCX_KICK_IDLE);

	/*
	 * @prev_cid has been kicked already, speculately kick a different idle
	 * cid, so the task can be immediately consumed elsewhere if prev_cid is
	 * busy.
	 */

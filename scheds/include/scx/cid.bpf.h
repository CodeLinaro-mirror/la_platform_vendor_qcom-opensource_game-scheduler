 ...
 * @nr_slots: number of slots, at least the scheduler's cid count
 *
 * The shared arena init calls this once. A scheduler with its own arena calls
 * it before the first distribute pick. Return 0 on success, -ENOMEM if the
 * allocation fails.
 */
static __always_inline int cmask_distribute_init(void *map, u32 nr_slots)
{
	u64 size = nr_slots * sizeof(*cmask_distribute_rotors);
